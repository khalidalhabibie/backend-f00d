...
# DATABASE_URL = postgres://postgres:password@thena-postgres/postgres?sslmode=disable
...
migrate.new:
	@read line;migrate create -ext sql -dir $(MIGRATIONS_FOLDER) -seq $$line

migrate:
	migrate -path $(MIGRATIONS_FOLDER) -database "$(DATABASE_URL)" up 1
...
docker.stop.fiber:
	docker stop thena-fiber

docker.stop.postgres:
	docker stop thena-postgres

docker.stop.redis:
	docker stop thena-redis
