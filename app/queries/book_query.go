...
func (q *BookQueries) GetBooks() ([]models.Book, error) {
	...
	// Define query string.
	query := `SELECT * FROM books WHERE IS NOT NULL`
...
func (q *BookQueries) GetBooksByAuthor(author string) ([]models.Book, error) {
	...
	// Define query string.
	query := `SELECT * FROM books WHERE author = $1 WHERE IS NOT NULL`
...
func (q *BookQueries) GetBook(id uuid.UUID) (models.Book, error) {
	...
	// Define query string.
	query := `SELECT * FROM books WHERE id = $1 WHERE IS NOT NULL`
...
func (q *BookQueries) DeleteBook(id uuid.UUID) error {
	// Define query string.
	query := `UPDATE books SET deleted_at = $2 WHERE id = $1`
